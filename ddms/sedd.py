...
class EulerScheduler(Scheduler):
    ...
    def step(self, output, xt, t, step_size, rev_rate=None, generator=None, if_last=False, if_student=False):
        if rev_rate is None:
            sigma = self.sigma(t)
...
class AnalyticScheduler(Scheduler):
    ...
    def step(self, output, xt, t, step_size, generator=None, if_last=False, **kwargs):
        ...
        return SchedulerOutput(xt, probs)
...
def sample_categorical(categorical_probs, eps=1e-6, generator=None):
    ...
    # return F.gumbel_softmax(categorical_probs / gumbel_noise, tau=1.0, hard=True,dim=-1 )
